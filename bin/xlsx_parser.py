...
import constants
from constants.common import month_arr
from data.excel import Gstr2bHeaders
...
class GstrParser:
    ...
    def get_month_num(month_str):
        m_arr = [month[:3] for month in month_arr]
        return m_arr.index(month_str)
...
class Gstr2bParser(GstrParser):
    def __init__(self, progress_bar, log_viewer):
        super(Gstr2bParser, self).__init__(progress_bar, log_viewer)
        ...
        while True:
            ...
            for index, d_type in enumerate(sheet_header):
                ...
                try:
                    ...
                    if d_type == t_header.SUBMITTED:
                        val = "SUBMITTED" \
                            if val.lower() in ["y", "yes"] else "NOT_SUBMITTED"
                    data_row[index] = val
                except ValueError:
                    pass

            if t_header.GST2_YRM in sheet_header:
                gst2_yrm_index = sheet_header.index(t_header.GST2_YRM)
                gst2_yrm = data_row[gst2_yrm_index]
                gst2_yrm = "20" + str(gst2_yrm[4:]) \
                           + str(self.get_month_num(gst2_yrm[:3])).zfill(2)
                data_row[gst2_yrm_index] = gst2_yrm
            if t_header.DOWNLOAD in sheet_header:
                download_index = sheet_header.index(t_header.DOWNLOAD)
                data_row[download_index] = \
                    str(data_row[download_index]).replace(' ', "-")
